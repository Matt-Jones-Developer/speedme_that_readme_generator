...
const setupHeadings = async () => {
  ...
  const { headings } = await inquirer.prompt([
    ...
    // [TODO] - dataObject time!

    // I want these an there own async function
    // async function proQuestions() {
    // each answer stored to responses object! {}
    // const responses = {};
    // }
    // each prompt has it's own await
    ...
    // we want to call these AFTER the baseQuestions!
    // console.log("OK.  Lets run through the headings first, then we'll add a title, summary and license next!")
    {
      type: 'input',
      name: 'about',
    ...
    // condtional: IF User Story chosen, generateUserStory(answers) and add Motivation/Challenges sub-headings
    {
      type: 'input',
      name: 'story',
  ...
  // const mainContent = await inquirer.prompt(headings);
  // const mainContent = [];
  // answers.name.push(mainContent)
...
const generateMainContentsSection = (chosenHeadings) => {
  ...
  // all the selected headings for each
...
// next issue! How to add content ONLY if user had that heading selected?
